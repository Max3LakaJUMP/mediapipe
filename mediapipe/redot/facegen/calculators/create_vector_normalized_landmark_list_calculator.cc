...
class CreateVectorNormalizedLandmarkListCalculator : public CalculatorBase {
 public:
  ...
  ::mediapipe::Status Process(CalculatorContext* cc) override {
    ...
    std::vector<NormalizedLandmarkList> output_vector;
    for (int i = 0; i < cc->Inputs().NumEntries(); ++i) {
    ...
    cc->Outputs().Index(0).AddPacket(
        MakePacket<std::vector<NormalizedLandmarkList>>(output_vector).At(cc->InputTimestamp()));
    return ::mediapipe::OkStatus();
  }
...
REGISTER_CALCULATOR(CreateVectorNormalizedLandmarkListCalculator);
